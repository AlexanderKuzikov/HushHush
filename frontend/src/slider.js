import { OpenFolderDialog, GetImages, GetConfig, SetInterval, SetShuffle, SetLoop, GetLastFolder } from '../../wailsjs/go/main/App.js'
...
export class Slider {
  ...
  async init() {
    ...
    // Пытаемся загрузить последнюю папку
  ...
  showCurrent() {
    ...
    // Запрашиваем предзагрузку следующих 3 кадров
    const nextPaths = []
    for (let i = 1; i <= 3; i++) {
      const nextIdx = (this.index + i) % this.images.length
      const ni = this.shuffle ? this.shuffleOrder[nextIdx] : nextIdx
      nextPaths.push(this.images[ni])
    }
    // PreloadImages вызываем без await — фоновая задача
    import('../../wailsjs/go/main/App.js').then(({ PreloadImages }) => {
  ...
  bindEvents() {
    ...
    document.getElementById('btn-folder').addEventListener('click', () => this.openFolder())
    document.getElementById('btn-prev').addEventListener('click', () => { this.prev(); this.resetTimer() })
    document.getElementById('btn-next').addEventListener('click', () => { this.next(); this.resetTimer() })
    document.getElementById('btn-play').addEventListener('click', () => this.togglePlay())
    document.getElementById('btn-shuffle').addEventListener('click', () => this.toggleShuffle())
    document.getElementById('btn-fullscreen').addEventListener('click', () => this.ui.toggleFullscreen())
    ...
    document.addEventListener('keydown', (e) => {
      switch (e.key) {
        case 'ArrowRight': case ' ': e.preventDefault(); this.next(); this.resetTimer(); break
        case 'ArrowLeft':            e.preventDefault(); this.prev(); this.resetTimer(); break
        case '+': case '=':          this.setInterval(Math.min(this.interval + 1, 3600)); this.ui.syncControls(this); break
        case '-':                    this.setInterval(Math.max(this.interval - 1, 1));    this.ui.syncControls(this); break
        case 'f': case 'F':          this.ui.toggleFullscreen(); break
    ...
    document.addEventListener('drop', async (e) => {
      ...
      if (item?.kind === 'file') {
        ...
        if (entry?.isDirectory) {
          await this.loadFolder(entry.fullPath)
        }
      }
    })
  }
